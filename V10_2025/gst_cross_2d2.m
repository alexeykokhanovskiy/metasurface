...
% =========================
lam_N = 2500;
lambda   = linspace(1, 6, lam_N);   % 2–3 µm
...
nn = [11, 11];                    % increase for convergence tests

% Normal incidence (θ = 0 → k_parallel = 0)
k_parallel  = 0;
angle_delta = 0;                % azimuth irrelevant at normal incidence
...
% Horizontal arm:
Lx = 3.5;        % total length along x  (µm)
Wx = 0.8;        % total width  along y  (µm)

% Vertical arm:
Ly = 3.5;        % total length along y  (µm)
Wy = 0.8;        % total width  along x  (µm)

% Number of points to polygonize circles (for ellipse primitive)
Nf = 12;         % Nf >= 8 recommended for smooth fillets
...
parfor il = 1:nPoints
    ...
    % ---------- Stadium Horizontal Arm ----------
    % Radius of end caps
    R_h  = Wx/2;
    Lc_h = Lx - 2*R_h;             % central rectangle length
    x0_h = Lc_h/2;                 % center position of circles

    % Check geometry sanity
    if Lc_h <= 0
        error('Horizontal arm: Lx too small for given Wx (Lc_h <= 0).');
    end

    Horz = {
        % central rectangle (horizontal bar)
        [0, 0, Lc_h, Wx, n_GSTc_c(il), 1], ...
        % left circular end cap
        [-x0_h, 0, 2*R_h, 2*R_h, n_GSTc_c(il), Nf], ...
    ...
    Vert = {
        ...
        [0, 0, Wy, Lc_v, n_GSTc_c(il), 1], ...
        % bottom circular end cap
        [0, -y0_v, 2*R_v, 2*R_v, n_GSTc_c(il), Nf], ...
        % top circular end cap
        [0,  y0_v, 2*R_v, 2*R_v, n_GSTc_c(il), Nf]
    };

    % -- Texture 2: union of both arms in air background --
    textures{2} = [{n_GSTa_c(il)}, Horz{:}, Vert{:}];
